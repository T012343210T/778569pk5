...
public class ScanFragment extends Fragment {
    ...
    private void startWifiScan() {
        ...
        wifiReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if (getContext() != null) getContext().unregisterReceiver(this);
                wifiReceiver = null;
            }
        };
        ...
        if (!started) {
            // اسکن شاید به‌خاطر محدودیت اندروید ۹+ رد شده باشد؛ نتایج قبلی موجود را نشان بده
            renderWifiResults();
            progressWifi.setVisibility(View.GONE);
            if (wifiReceiver != null && getContext() != null) {
                getContext().unregisterReceiver(wifiReceiver);
                wifiReceiver = null;
            }
            toast("اسکن جدید محدود شد؛ آخرین نتایج نمایش داده شد");
        }
    }
    ...
    private void renderWifiResults() {
        ...
        if (results == null || results.isEmpty()) {
            addEmptyRow(wifiContainer, "شبکه‌ای پیدا نشد");
            return;
        }
